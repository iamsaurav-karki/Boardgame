  ...
  stages {
      ...
      steps {
        script {
            withEnv(["SNYK_TOKEN=${env.SNYK_TOKEN}"]) {
                sh 'snyk test --severity-threshold=high --json-file-output=snyk-source-scan.json'
            }
        }
      }
    }
    ...
    stage('Snyk Docker Image Scan') {
      steps {
        script {
            withEnv(["SNYK_TOKEN=${env.SNYK_TOKEN}"]) {
                sh "snyk test --docker sauravkarki/bankappnew:${params.DOCKER_TAG} --file=Dockerfile --severity-threshold=high --json-file-output=snyk-docker-scan.json"
            }
        }
      }
    }
    ...
    stage('Publish Snyk HTML Report') {
      steps {
        script {
            sh 'npm install -g snyk-to-html'
            sh 'snyk-to-html -i snyk-source-scan.json -o snyk-source-scan.html'
            sh 'snyk-to-html -i snyk-docker-scan.json -o snyk-docker-scan.html'
            publishHTML target: [
                allowMissing: false,
                alwaysLinkToLastBuild: true,
                keepAll: true,
                reportDir: '',
                reportFiles: 'snyk-source-scan.html',
                reportName: 'Snyk Source Scan Report'
            ]
            publishHTML target: [
                allowMissing: false,
                alwaysLinkToLastBuild: true,
                keepAll: true,
                reportDir: '',
                reportFiles: 'snyk-docker-scan.html',
                reportName: 'Snyk Docker Scan Report'
            ]
        }
      }
    }
